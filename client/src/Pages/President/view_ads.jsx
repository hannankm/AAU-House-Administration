...
const PresidentAuthorizationPage = () => {
  const [ads, setAds] = useState([]);

  useEffect(() => {
    const fetchAdsForAuthorization = async () => {
      try {
        const response = await fetch("http://localhost:5000/ads/president/");
        if (response.ok) {
    ...
    <div className="flex h-screen">
      ...
      <div>
        <h2 className="text-2xl font-medium text-center my-4">
          Advertisements for President Authorization
        </h2>
              ...
              {ads.map((ad) => (
                <tr key={ad.ad_id} className="hover:bg-gray-100">
                  ...
                  <td>
                    <Link
                      to={{
                        pathname: `/president/adDetails/${ad.ad_id}`,
                        state: { adObject: ad },
                      }}
                      key={ad.ad_id}
...
export default PresidentAuthorizationPage;
