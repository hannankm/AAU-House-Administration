...
const AdDetails2 = () => {
  ...
  const handleApprove = async (adId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/ads/president/authorize/${adId}`,
        {
  ...
  const handleRejectClick = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/ads/president/reject/${adDetails.ad_id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        console.log("Advertisement rejected by president:", data);
  ...
  return (
    <div className="flex h-screen">
      <div className=" w-1/5">
        <Sidebar active={"houses"} role="president" />
      </div>
